// lib/pages/riwayat_tiket_list.dart
...
import 'riwayat_detail_page.dart'; // Mengarah ke RiwayatTicketDetailPage

class RiwayatTiketList extends StatefulWidget {
  const RiwayatTiketList({super.key});

  @override
  State<RiwayatTiketList> createState() => _RiwayatTiketListState();
}

class _RiwayatTiketListState extends State<RiwayatTiketList> {
  ...
  Future<void> _fetchHistoryData() async {
    ...
    try {
      ...
      // --- PERBAIKAN LOGIKA FILTER (UNTUK RIWAYAT) ---
      final List<Map<String, dynamic>> filteredItems = allItems.where((item) {
        
        final String status = (item['status'] ?? 'pending').toLowerCase();

        // --- ATURAN 1: Tampilkan jika tiket GAGAL, KADALUARSA, atau DIBATALKAN ---
        // Ini adalah riwayat, jadi kita tampilkan langsung tidak peduli tanggalnya.
        if (status == 'canceled' || status == 'failed' || status == 'expired') {
          return true;
        }

        // --- ATURAN 2: Untuk 'success' atau 'pending', cek tanggal kunjungannya ---
        if (status == 'success' || status == 'pending') {
            ...
            if (dateString == null) {
              return false; // Tidak ada tanggal, jangan tampilkan
            }

            try {
              final visitDate = DateTime.parse(dateString);
              // Tampilkan HANYA jika tanggal kunjungan SUDAH LEWAT
              return visitDate.isBefore(today); 
            } catch (e) {
              return false; // Gagal parse tanggal, jangan tampilkan
            }
        }

        // Jika ada status lain (misal 'challenge' dll), abaikan
        return false;
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'Riwayat tiket masih kosong', // Teks empty state sesuai file ini
              style: TextStyle(
    ...
    return InkWell(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            // Mengarah ke RiwayatTicketDetailPage
            builder: (context) => RiwayatTicketDetailPage(ticket: ticket),
          ),
        );
      },
      child: Card(
